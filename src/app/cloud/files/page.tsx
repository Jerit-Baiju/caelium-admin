...
import { FiAlertCircle, FiCloud, FiDownload, FiEye, FiFile, FiHardDrive, FiLoader, FiSearch, FiUser } from 'react-icons/fi';
...
export default function CloudFilesPage() {
  ...
  const loadImagePreviews = useCallback(
    async (fileList: CloudFile[]) => {
      if (!authTokens?.access) return;

      const imageFiles = fileList.filter((file) => file.mime_type.startsWith('image/') && file.upload_status === 'completed');
      ...
      <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className='flex flex-col gap-4'>
        <div className='flex items-center gap-3'>
          ...
          <div>
            ...
            <p className='text-muted-foreground'>Manage all uploaded files across the platform with decryption support</p>
          </div>
        </div>
                ...
                {users.map((user) => (
                  <SelectItem key={user.id} value={user.id.toString()}>
                    <div className='flex items-center gap-2'>
                      {user.avatar ? (
                        <Image src={user.avatar} alt={user.name} width={16} height={16} className='rounded-full' />
                      ) : (
        ...
        <div className='bg-card border border-border rounded-lg p-4'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='text-lg font-semibold'>{files.filter((f) => f.upload_status === 'completed').length}</p>
            </div>
          </div>
        </div>
        ...
        <div className='bg-card border border-border rounded-lg p-4'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='text-lg font-semibold'>{files.filter((f) => f.upload_status === 'pending').length}</p>
            </div>
          </div>
        </div>
        ...
        <div className='bg-card border border-border rounded-lg p-4'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='text-lg font-semibold'>{files.filter((f) => f.upload_status === 'failed').length}</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div
        ...
        {loading ? (
          ...
          <div className='text-center py-12'>
            ...
            <p className='text-muted-foreground'>All files are encrypted and can be decrypted for download</p>
          </div>
        ) : (
          <div className='p-6'>
      ...
      {pagination.totalPages > 1 && (
        <motion.div
          ...
          <p className='text-sm text-muted-foreground'>
            Showing {(pagination.currentPage - 1) * 20 + 1} to {Math.min(pagination.currentPage * 20, pagination.count)} of{' '}
          ...
          <div className='flex gap-2'>
            ...
            <span className='px-3 py-1 bg-primary/10 text-primary rounded-md'>{pagination.currentPage}</span>
